        ...
        <a :href="repo.html_url">
          ...
          >
            <div class="w-10/12 sm:w-full">
              <div class="font-bold">
                {{ repo.name
                  ...
                  class="ml-1"
                  v-if="repo.fork"
                  name="i-mdi-source-branch"
                  dynamic
                />
              </div>
              <div>{{ repo.description }}</div>
              <div
                class="flex items-center space-x-1 justify-start"
                v-if="repo.homepage"
              >
                <UIcon class="flex" name="i-heroicons-globe-alt-solid" />
                <a
                  class="hover:underline truncate sm:w-full w-3/5"
                  :href="repo.homepage"
                  >{{ repo.homepage }}</a
                >
              </div>
            ...
            <div class="flex items-center">
              {{ repo.stargazers_count }}<UIcon name="i-heroicons-star-solid" />
            </div>
          </div>
        </a>
      </div>
...
const { data, error, pending } = await useFetch(
  "https://api.github.com/users/cferreras/repos"
);
